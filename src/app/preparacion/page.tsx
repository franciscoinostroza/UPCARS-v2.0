            ...
            <div className="card w-full max-w-lg animate-fade-up p-5" style={{ background: 'var(--bg-card)', maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex flex-wrap gap-3 mt-1">
                  <span onClick={(e) => { e.stopPropagation(); const nv = !selected.limpiezaInterior; const tk = new URLSearchParams(window.location.search).get('token') || ''; setRecords(p => p.map(r => r.id === selected.id ? { ...r, limpiezaInterior: nv } as PrepItem : r)); setSelected(p => p ? { ...p, limpiezaInterior: nv } : null); fetch(`/api/preparacion/${selected.id}?token=${tk}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ limpiezaInterior: nv }) }).catch(() => {}) }} className="cursor-pointer select-none" style={{ color: selected.limpiezaInterior ? '#22c55e' : 'var(--text-muted)' }}>{selected.limpiezaInterior ? '✅ Interior' : '⬜ Interior'}</span>
                  <span onClick={(e) => { e.stopPropagation(); const nv = !selected.limpiezaExterior; const tk = new URLSearchParams(window.location.search).get('token') || ''; setRecords(p => p.map(r => r.id === selected.id ? { ...r, limpiezaExterior: nv } as PrepItem : r)); setSelected(p => p ? { ...p, limpiezaExterior: nv } : null); fetch(`/api/preparacion/${selected.id}?token=${tk}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ limpiezaExterior: nv }) }).catch(() => {}) }} className="cursor-pointer select-none" style={{ color: selected.limpiezaExterior ? '#22c55e' : 'var(--text-muted)' }}>{selected.limpiezaExterior ? '✅ Exterior' : '⬜ Exterior'}</span>
                  <span onClick={(e) => { e.stopPropagation(); const nv = !selected.fotografiaAnuncio; const tk = new URLSearchParams(window.location.search).get('token') || ''; setRecords(p => p.map(r => r.id === selected.id ? { ...r, fotografiaAnuncio: nv } as PrepItem : r)); setSelected(p => p ? { ...p, fotografiaAnuncio: nv } : null); fetch(`/api/preparacion/${selected.id}?token=${tk}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fotografiaAnuncio: nv }) }).catch(() => {}) }} className="cursor-pointer select-none" style={{ color: selected.fotografiaAnuncio ? '#22c55e' : 'var(--text-muted)' }}>{selected.fotografiaAnuncio ? '📸 Foto ✅' : '📸 Foto'}</span>
                  <span onClick={(e) => { e.stopPropagation(); const nv = !selected.registrarInicio; const tk = new URLSearchParams(window.location.search).get('token') || ''; setRecords(p => p.map(r => r.id === selected.id ? { ...r, registrarInicio: nv } as PrepItem : r)); setSelected(p => p ? { ...p, registrarInicio: nv } : null); fetch(`/api/preparacion/${selected.id}?token=${tk}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ registrarInicio: nv }) }).catch(() => {}) }} className="cursor-pointer select-none" style={{ color: selected.registrarInicio ? '#22c55e' : 'var(--text-muted)' }}>{selected.registrarInicio ? '🟢 Inicio ✅' : '🟢 Inicio'}</span>
                  <span onClick={(e) => { e.stopPropagation(); const nv = !selected.registrarFin; const tk = new URLSearchParams(window.location.search).get('token') || ''; setRecords(p => p.map(r => r.id === selected.id ? { ...r, registrarFin: nv } as PrepItem : r)); setSelected(p => p ? { ...p, registrarFin: nv } : null); fetch(`/api/preparacion/${selected.id}?token=${tk}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ registrarFin: nv }) }).catch(() => {}) }} className="cursor-pointer select-none" style={{ color: selected.registrarFin ? '#22c55e' : 'var(--text-muted)' }}>{selected.registrarFin ? '🔴 Fin ✅' : '🔴 Fin'}</span>
                </div>
              ...
              <button onClick={() => { setConfirmDelete(selected.id); return }} className="w-full text-[11px] font-semibold py-2 rounded mt-4" style={{ background: 'rgba(239,68,68,0.12)', color: '#ef4444', cursor: 'pointer' }}>🗑 Eliminar</button>
            </div>
          </div>
        )}
