          ...
          <div className="card overflow-x-auto animate-fade-up" style={{ animationDelay: '75ms' }}>
            {records.length === 0 ? <div className="p-8 text-center"><p className="text-sm" style={{ color: 'var(--text-muted)' }}>Sin registros</p></div> : (
              <table className="w-full text-xs sm:text-sm">
                ...
                <tbody>
                  {records.map(r => (
                    <tr key={r.id} onClick={() => { setSelected(r); setEditObs(r.observaciones); setEditTrabajos(r.trabajosSolicitados) }} style={{ borderBottom: '1px solid var(--border)', cursor: 'pointer' }} className="hover:opacity-80">
                      ...
                      <td className="text-right p-2 sm:p-3" style={{ color: 'var(--text-secondary)' }}>{r.diasFuera ?? '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
