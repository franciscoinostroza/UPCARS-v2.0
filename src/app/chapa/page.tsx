        ...
        {vista === 'kanban' ? (
          ...
          <div className="space-y-2 animate-fade-up">
            ...
            : records.map(r => {
              ...
              return (
                <div key={r.id} onClick={() => { setSelected(r); setEditObs(r.observaciones); setEditTrabajos(r.trabajosSolicitados) }}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h3 className="text-sm font-semibold truncate" style={{ color: 'var(--text)' }}>{r.matricula || r.vehiculoNombre}</h3>
                      <span className="text-[10px] font-semibold px-2 py-0.5 rounded" style={{ background: ec.bg, color: ec.text }}>{r.estado || 'Sin estado'}</span>
                      {r.diasFuera != null && <span className="text-[9px] font-medium" style={{ color: 'var(--text-muted)' }}>📆 {r.diasFuera}d</span>}
                    </div>
                    <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px]" style={{ color: 'var(--text-secondary)' }}>
                      ...
                      {r.proveedorNombre && <span>🏭 {r.proveedorNombre}</span>}
                      {r.costeTotal != null && <span style={{ color: 'var(--accent-blue)' }}>💰 {fmtEuro(r.costeTotal)}</span>}
                      {r.fechaSalida && <span>📅 Sal: {fmtDate(r.fechaSalida)}</span>}
                      {r.fechaRetorno && <span>📅 Ret: {fmtDate(r.fechaRetorno)}</span>}
                    </div>
                  </div>
                </div>
              )
