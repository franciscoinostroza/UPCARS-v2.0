                    ...
                    {selected.tarifasLeasing.filter(f => f.name !== 'Imagen').concat(selected.tarifasVnVo).map((f, i) => {
                      ...
                      return (
                        <div key={f.name} className="card p-2" style={{ background: 'var(--bg-pill)' }}>
                          ...
                          {isOffice && <a href={url} target="_blank" rel="noopener noreferrer" className="block text-[11px] py-1 font-medium" style={{ color: 'var(--accent-blue)' }}>📥 {f.name}</a>}
                          {!isOffice && <a href={url} target="_blank" rel="noopener noreferrer" className="inline-block mt-1 text-[10px] font-medium" style={{ color: 'var(--text-secondary)' }}>📥 Descargar</a>}
                        </div>
                      )
                    })}
                  </div>
            ...
            <div className="card w-full max-w-lg animate-fade-up p-5" style={{ background: 'var(--bg-card)', maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button onClick={() => setConfirmDelete(selected.id)} className="flex-1 text-[11px] font-semibold py-2 rounded" style={{ background: 'rgba(239,68,68,0.12)', color: '#ef4444', cursor: 'pointer' }}>🗑 Eliminar</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
            <div className="card p-5 max-w-sm animate-fade-up" style={{ background: 'var(--bg-card)' }}>
              <p className="text-sm font-semibold mb-4 text-center" style={{ color: 'var(--text)' }}>🗑 ¿Eliminar esta financiera?</p>
              <div className="flex gap-2">
                <button onClick={async () => {
